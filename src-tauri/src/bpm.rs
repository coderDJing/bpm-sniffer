...
use crate::logging::EMIT_TEXT_LOGS;
use serde::Serialize;
...
impl BpmEstimator {
    pub fn new(sample_rate: u32) -> Self {
        ...
        if EMIT_TEXT_LOGS {
            if is_log_zh() {
                eprintln!("[初始化] 估计器 sr={} 下采样率={}Hz", sample_rate, 200.0f32);
            } else {
                eprintln!(
                    "[INIT] BpmEstimator sr={} ds_rate={}",
                    sample_rate, 200.0f32
                );
            }
        }
        this
    ...
    pub fn push_frames(&mut self, frames: &[f32]) -> Option<BpmEstimate> {
        ...
        if !frames.is_empty() {
            ...
            if db - self.last_input_rms_db > 6.0 {
                ...
                if EMIT_TEXT_LOGS {
                    if is_log_zh() {
                        eprintln!(
                            "[复位] 输入电平跳变 {:.1} -> {:.1}，重置滤波并裁剪缓冲",
                            self.last_input_rms_db, db
                        );
                    } else {
                        eprintln!(
                            "[RST] input_db jump {:.1} -> {:.1}, reset filters & trim buffer",
                            self.last_input_rms_db, db
                        );
                    }
                }
        ...
        if self.buf.len() < (self.ds_rate as usize) * 1 {
            return None;
        }
        ...
        if rms < 4e-4 {
            if EMIT_TEXT_LOGS {
                if is_log_zh() {
                    eprintln!("[门限] 下采样RMS={:.6} 低于阈值，跳过", rms);
                } else {
                    eprintln!("[GATE] ds_rms={:.6} below threshold, skip", rms);
                }
            }
            return None;
        }
        ...
        let eval_slice = |xs: &[f32]| -> Option<(f32, f32, f32, f32, f32)> {
            if xs.len() <= max_lag + 1 {
                if VERBOSE_BPM_DEBUG {
                    eprintln!("[FILT] too_short xs_len={} need>{}", xs.len(), max_lag + 1);
                }
                return None;
            }
            // 首尾静音裁剪：阈值=3%峰值
            ...
            if max_v <= 1e-7 {
                if VERBOSE_BPM_DEBUG {
                    eprintln!("[FILT] max_v too small max_v={:.3e}", max_v);
                }
                return None;
            }
            let thr = (0.03 * max_v).max(1e-7);
            let mut i0 = 0usize;
            while i0 < xs.len() && xs[i0].abs() < thr {
                i0 += 1;
            }
            let mut i1 = xs.len().saturating_sub(1);
            while i1 > i0 && xs[i1].abs() < thr {
                i1 -= 1;
            }
            if i1 <= i0 {
                if VERBOSE_BPM_DEBUG {
                    eprintln!("[FILT] trim collapse i0={} i1={}", i0, i1);
                }
                return None;
            }
            let slice = &xs[i0..=i1];
            ...
            if n >= 3 {
                for i in 0..n {
                    let w = 0.5
                        * (1.0 - (2.0 * std::f32::consts::PI * i as f32 / (n as f32 - 1.0)).cos());
                    x[i] *= w as f32;
                }
            }
            ...
            for i in 1..(x.len().saturating_sub(1)) {
                if x[i] > dyn_thr && x[i] >= x[i - 1] && x[i] > x[i + 1] {
                    if let Some(&last) = peaks.last() {
                        if i.saturating_sub(last) < min_sep {
                            continue;
                        }
                    }
                    let w = min_sep.max(4);
            ...
            if peaks.len() < 2 {
                if VERBOSE_BPM_DEBUG {
                    eprintln!(
                        "[FILT] peaks<2 dyn_thr={:.4} peak_thr={:.4} max_x={:.4}",
                        dyn_thr, peak_thr, max_x
                    );
                }
                return None;
            }
            ...
            if peaks.len() >= 4 {
                ...
                if mean > 1e-3 {
                    let var = iois.iter().map(|v| (v - mean) * (v - mean)).sum::<f32>()
                        / iois.len() as f32;
                    let std = var.sqrt();
                    cv = (std / mean).min(2.0);
                }
                if cv > 0.65 {
                    if VERBOSE_BPM_DEBUG {
                        eprintln!("[FILT] cv high cv={:.3} thr=0.65", cv);
                    }
                    return None;
                }
            }
            ...
            if peakiness < 1.25 {
                if VERBOSE_BPM_DEBUG {
                    eprintln!("[FILT] peakiness low p95/p50={:.3}", peakiness);
                }
                return None;
            }
            ...
            if peaks_per_sec < 0.20 || peaks_per_sec > 14.0 {
                if VERBOSE_BPM_DEBUG {
                    eprintln!("[FILT] density out pps={:.2}", peaks_per_sec);
                }
                return None;
            }

            // 峰数量：窗口越长要求越高（4s 目标至少 4 个峰）
            let min_peaks = if win_sec_here >= 3.5 {
                4
            } else if win_sec_here >= 2.0 {
                2
            } else {
                2
            };
            if peaks.len() < min_peaks {
                if VERBOSE_BPM_DEBUG {
                    eprintln!(
                        "[FILT] not enough peaks cnt={} need>={} win={:.2}s",
                        peaks.len(),
                        min_peaks,
                        win_sec_here
                    );
            ...
            let (start_i, end_i) = {
                ...
                let mut pad = if iois_for_pad.is_empty() {
                    ((self.ds_rate * 60.0 / self.max_bpm).round() as usize) / 2
                } else {
                    let mut s = iois_for_pad.clone();
                    s.sort();
                    (s[s.len() / 2] as f32 * 0.75) as usize
                };
                if pad < 2 {
                    pad = 2;
                }
                let s = peaks.first().copied().unwrap_or(0);
                let e = peaks.last().copied().unwrap_or(x.len().saturating_sub(1));
                (
                    s.saturating_sub(pad),
                    (e + pad).min(x.len().saturating_sub(1)),
                )
            };
            ...
            let xw: &[f32] = if xw_crop.len() < min_keep {
                if slice.len() >= min_keep {
                    if VERBOSE_BPM_DEBUG {
                        eprintln!(
                            "[FALLBACK] use un-cropped slice len={} instead of cropped len={}",
                            slice.len(),
                            xw_crop.len()
                        );
                    }
                    ...
                    if VERBOSE_BPM_DEBUG {
                        eprintln!("[FILT] xw too short after peak-crop len={} and raw slice len={} both < {}", xw_crop.len(), slice.len(), min_keep);
                    }
                    return None;
                }
            } else {
            ...
            let eval_r_at = |lag: usize| -> f32 {
                if lag <= min_lag || lag >= max_lag || xw.len() <= lag {
                    return 0.0;
                }
                let mut num = 0.0f32;
                let mut e1 = 0.0f32;
            ...
            let harm_penalty = if family_min < r_primary * 0.35 {
                if VERBOSE_BPM_DEBUG {
                    eprintln!(
                        "[HARM] penalize r_pri={:.3} r_half={:.3} r_dbl={:.3}",
                        r_primary, r_half, r_dbl
                    );
                }
                0.90
            } else {
            ...
            // 守卫
            if best_lag <= min_lag + 1 && best_score < 0.20 {
                if VERBOSE_BPM_DEBUG {
                    eprintln!(
                        "[FILT] edge lag score too low lag={} score={:.3}",
                        best_lag, best_score
                    );
                }
                return None;
            ...
            if best_score < 0.08 {
                if VERBOSE_BPM_DEBUG {
                    eprintln!("[FILT] best_score too low {:.3}", best_score);
                }
                return None;
            }
            ...
            if margin < 0.010 {
                if VERBOSE_BPM_DEBUG {
                    eprintln!("[FILT] margin too small {:.4}", margin);
                }
                return None;
            }
            let mut confidence =
                (best_score * ratio.sqrt()).clamp(0.0, 0.95).powf(0.85) * harm_penalty;
            // 置信度调制
            ...
            if confidence < 0.05 {
                if VERBOSE_BPM_DEBUG {
                    eprintln!("[FILT] confidence too low {:.3}", confidence);
                }
                return None;
            }
        ...
        // 长窗（最多12s，实际为 buf 全部）
        let long_est = eval_slice(&self.buf.as_slices().0).or_else(|| {
        ...
        let (bpm, confidence, from_short, win_sec) = match (long_est, short_est) {
            (Some((bl, cl, _, _, wl)), Some((bs, cs, _, _, ws))) => {
                ...
                // 短窗一致性计数
                if let Some(prev) = self.last_short_bpm {
                    if (bs - prev).abs() / bs.max(1e-6) <= 0.03 {
                        self.short_consistency = self.short_consistency.saturating_add(1);
                    } else {
                        self.short_consistency = 1;
                    }
                } else {
                    self.short_consistency = 1;
                }
                ...
                if prefer_short {
                    if EMIT_TEXT_LOGS {
                        if is_log_zh() {
                            eprintln!("[选择] 短窗 bs={:.1} cs={:.2} wl={:.1}s 分歧={} 连续={} 对比 长窗 bl={:.1} cl={:.2}", bs, cs, ws, diverge, self.short_consistency, bl, cl);
                        } else {
                    ...
                    if EMIT_TEXT_LOGS {
                        if is_log_zh() {
                            eprintln!("[选择] 长窗 bl={:.1} cl={:.2} wl={:.1}s 对比 短窗 bs={:.1} cs={:.2} 分歧={} 连续={}", bl, cl, wl, bs, cs, diverge, self.short_consistency);
                        } else {
        ...
        if EMIT_TEXT_LOGS {
            if is_log_zh() {
                eprintln!(
                    "[输出] bpm={:.1} 置信度={:.2} 源={} 窗口={:.1}s",
                    bpm,
                    confidence,
                    if from_short { 'S' } else { 'L' },
                    win_sec
                );
            } else {
                eprintln!(
                    "[OUT] bpm={:.1} conf={:.2} src={} win={:.1}s",
                    bpm,
                    confidence,
                    if from_short { 'S' } else { 'L' },
                    win_sec
...
impl SimpleBackend {
    pub fn new(sample_rate: u32) -> Self {
        Self {
            inner: BpmEstimator::new(sample_rate),
        }
    }
    pub fn process_frames(&mut self, frames: &[f32]) -> Option<BpmEstimate> {
