    ...
    pub fn start_loopback() -> Result<(Self, Receiver<Vec<f32>>, Receiver<u32>)> {
        ...
        thread::spawn(move || unsafe {
            ...
            loop {
                ...
                if pwfx.is_null() {
                    if !sent_init {
                        let _ = init_tx.send(Err(anyhow!("GetMixFormat returned null")));
                    }
                    thread::sleep(Duration::from_millis(300));
                    continue;
                }
                ...
                loop {
                    ...
                    let mut packet_len = match capture_client.GetNextPacketSize() {
                        Ok(n) => n,
                        Err(_) => break,
                    };
                    while packet_len > 0 {
                            ...
                            if !is_silent && !data_ptr.is_null() {
                                    ...
                                    && mix.wBitsPerSample == 16
                                {
                                    ...
                                    for frame in slice.chunks(channels) {
                                        ...
                                        let r = *frame.get(1).unwrap_or(&frame[0]) as f32 / 32768.0;
                                        mono.push((l + r) * 0.5);
                                    }
                                } else {
                        ...
                        packet_len = match capture_client.GetNextPacketSize() {
                            Ok(n) => n,
                            Err(_) => 0,
                        };
                    }
        ...
        let sample_rate = init_rx
            .recv()
            .map_err(|_| anyhow!("audio init channel closed"))??;
