...
pub fn run_capture(app: AppHandle) -> Result<()> {
    ...
    loop {
        ...
        if sc_hp_alpha_local == 0.0 || sc_lp_alpha_local == 0.0 {
            ...
            sc_hp_lp_prev = 0.0;
            sc_lp_prev = 0.0;
            sc_hp_alpha = sc_hp_alpha_local;
            sc_lp_alpha = sc_lp_alpha_local;
        }
        ...
        if is_silent {
            ...
            emit_friendly(
                &app,
                "检测到环境安静，BPM 为 0（等待声音）",
                "Silence detected. BPM is 0 (waiting for audio)",
            );
        ...
        if NORM_ENABLE {
            ...
            for &s in &frames_for_analysis {
                ...
                let hp = s - hp_lp;
                sc_hp_lp_prev = hp_lp;
                let lp = sc_lp_alpha * sc_lp_prev + (1.0 - sc_lp_alpha) * hp;
                sc_lp_prev = lp;
                sc_sumsq += lp * lp;
            }
            let sc_rms = (sc_sumsq / frames_for_analysis.len() as f32)
                .sqrt()
                .max(1e-9);
            ...
            let dyn_max_gain = if rhythm_ratio >= RHYTHM_RATIO_THR {
                NORM_MAX_GAIN_DB.max(NORM_MAX_GAIN_DB_EXT)
            } else {
                NORM_MAX_GAIN_DB.min(MAX_GAIN_DB_WHEN_LOW_RATIO)
            };
            if need_gain_db > dyn_max_gain {
                need_gain_db = dyn_max_gain;
            }
            if need_gain_db < NORM_MIN_GAIN_DB {
                need_gain_db = NORM_MIN_GAIN_DB;
            }
            let a = if need_gain_db > norm_gain_db_smooth {
                NORM_ATTACK
            } else {
                NORM_RELEASE
            };
            ...
            if lin_gain != 1.0 {
                for x in &mut frames_for_analysis {
                    *x *= lin_gain;
                }
            }
            if NORM_SOFT_K > 0.0 {
                for x in &mut frames_for_analysis {
                    let y = (NORM_SOFT_K * *x).tanh();
                    *x = y / NORM_SOFT_K;
                }
            }
        }
        ...
        if let Some(raw) = backend.process(&frames_for_analysis) {
            ...
            if last_from_short.map_or(true, |v| v != raw.from_short) {
                if raw.from_short {
                    emit_friendly(
                        &app,
                        "切换为短窗优先（更快跟随变化）",
                        "Switched to short-window (faster response)",
                    );
                } else {
                    emit_friendly(
                        &app,
                        "切换为长窗优先（更稳更准）",
                        "Switched to long-window (more stable)",
                    );
                }
            ...
            let (thr_hi, thr_lo) = if is_ultra_short {
                (0.15f32, 0.08f32)
            } else {
                (hi_th, lo_th)
            };
            ...
            if !tracking && hi_cnt >= 3 {
                ...
                if EMIT_TEXT_LOGS {
                    let txt = if is_log_zh() {
                        format!("[状态] 进入追踪 bpm={:.1} 置信度={:.2}", raw.bpm, conf)
                    } else {
                        format!("[STATE] tracking=ON  bpm={:.1} conf={:.2}", raw.bpm, conf)
                    };
                    eprintln!("{}", txt);
                    let log = BackendLog {
                        t_ms: now_ms(),
                ...
                emit_friendly(
                    ...
                    format!(
                        "Beat locked: ~{:.0} BPM (confidence {:.0}%)",
                        raw.bpm,
                        conf * 100.0
                    ),
                );
            ...
            if tracking && lo_cnt >= 2 {
                ...
                if EMIT_TEXT_LOGS {
                    let txt = if is_log_zh() {
                        format!("[状态] 退出追踪 置信度={:.2}", conf)
                    } else {
                        format!("[STATE] tracking=OFF conf={:.2}", conf)
                    };
                    eprintln!("{}", txt);
                    let log = BackendLog {
                        t_ms: now_ms(),
                ...
                emit_friendly(
                    &app,
                    "节拍暂不稳定，正在重新分析…",
                    "Beat unstable. Re-analyzing…",
                );
            }

            let state: &str = if tracking {
                "tracking"
            } else if ever_locked {
                "uncertain"
            } else {
            ...
            if let Some(base) = anchor_bpm {
                ...
                let try_cand = |val: f32,
                                kind: &'static str,
                                best_bpm: &mut f32,
                                best_err: &mut f32,
                                best_kind: &mut &'static str| {
                ...
                if best_bpm != disp && EMIT_TEXT_LOGS {
                    if is_log_zh() {
                        eprintln!(
                            "[谐波校正] {} -> {:.1} (基准={:.1}, 原始={:.1})",
                            best_kind, best_bpm, base, disp
                        );
                    } else {
                        eprintln!(
                            "[CORR] {} -> {:.1} (base={:.1}, raw={:.1})",
                ...
                if best_bpm != disp {
                    emit_friendly(
                        ...
                        format!(
                            "Harmonic correction: {} → {:.1} (ref {:.1}, raw {:.1})",
                            best_kind, best_bpm, base, disp
                        ),
                    );
                }
                disp = best_bpm;
                _corr_kind = best_kind;
            }
...

            {
                ...
                unsafe {
                    ...
                    if let Some(n) = LOCK_INT {
                        if conf >= 0.85 && (disp - n as f32).abs() >= 2.0 {
                            LOCK_INT = None;
                            LOCK_CNT = 0;
                            UNLOCK_CNT = 0;
                        }
                    ...
                    if conf >= 0.80 && within {
                        if let Some(n) = LOCK_INT {
                            if n == disp_round {
                                LOCK_CNT = LOCK_CNT.saturating_add(1);
                            } else {
                                LOCK_CNT = 1;
                        ...
                        if conf >= switch_conf {
                            let near_other =
                                (disp - disp_round as f32).abs() <= 0.4 && disp_round != n;
                            if near_other {
                                if ALT_INT == Some(disp_round) {
                                    ALT_CNT = ALT_CNT.saturating_add(1);
                                } else {
                                    ALT_INT = Some(disp_round);
                                    ALT_CNT = 1;
                                }
                        ...
                        if (disp - n as f32).abs() >= 8.0 {
                            dev_from_lock_cnt = dev_from_lock_cnt.saturating_add(1);
                        } else {
                            dev_from_lock_cnt = 0;
                        }
                    }
...

            {
                ...
                while let Some(&(_, t0)) = recent_ints_cand.front() {
                    if nowh.saturating_sub(t0) > 1500 {
                        recent_ints_cand.pop_front();
                    } else {
                        break;
                    }
                }
            ...
            if recent_none_flag && conf >= 0.50 {
                ...
                emit_friendly(
                    &app,
                    "从空段恢复，快速锁定中…",
                    "Recovered from none, fast relock…",
                );
            }
            if dev_from_lock_cnt >= 2 {
                ...
                emit_friendly(
                    &app,
                    "检测到与锁定值偏离，快速锁定中…",
                    "Deviation from locked value, fast relock…",
                );
                let nowh = now_ms();
                let mut counts: std::collections::HashMap<i32, usize> =
                    std::collections::HashMap::new();
                for (v, t) in recent_ints.iter().rev() {
                    if nowh.saturating_sub(*t) <= 1500 {
            ...
            while let Some(&(_, t0)) = stable_vals.front() {
                if now_t.saturating_sub(t0) > stable_win_ms {
                    stable_vals.pop_front();
                } else {
                    break;
                }
            }
                ...
                if let Ok(mut guard) = cell.lock() {
                    ...
                    if base_for_guard.is_none() {
                        if let Some(prev) = *guard {
                            if prev.bpm > 0.0 {
                                base_for_guard = Some(prev.bpm.round());
                            }
                        }
                    }
                    if allow_hard {
                        if let Some(base) = base_for_guard {
                            if tracking {
                                ...
                                if rel > 0.12 {
                                    let mut best = disp;
                                    let mut err = rel;
                                    let try_back = |v: f32, best: &mut f32, err: &mut f32| {
                                        if v >= 60.0 && v <= 200.0 {
                                            let e = (v - base).abs() / base.max(1e-6);
                                            if e < *err {
                                                *err = e;
                                                *best = v;
                                            }
                                    ...
                                    if err <= 0.08 {
                                        ...
                                        if EMIT_TEXT_LOGS {
                                            if is_log_zh() {
                                                eprintln!("[离群] 抑制显示 bpm={:.1} 基准={:.1} 相对误差={:.3}", disp, base, rel);
                                            } else {
                                                eprintln!("[OUTLIER] suppress show bpm={:.1} base={:.1} rel={:.3}", disp, base, rel);
                                            }
                                        }
                                        emit_friendly(
                                            &app,
                                            ...
                                            format!(
                                                "Ignored outlier: {:.1} BPM (deviation {:.0}%)",
                                                disp,
                                                rel * 100.0
                                            ),
                                        );
                                ...
                                if (disp > 180.0 || disp < 91.0) && conf >= 0.80 {
                                    ...
                                    if EMIT_TEXT_LOGS {
                                        if is_log_zh() {
                                            ...
                                            eprintln!("[OUTLIER] suppress out-of-range bpm={:.1} base={:.1}", disp, base);
                                        }
                                    }
                                    emit_friendly(
                                        &app,
                                        format!("放弃越界结果：{:.1} BPM（当前范围 91–180）", disp),
                                        format!(
                                            "Dropped out-of-range result: {:.1} BPM (range 91–180)",
                                            disp
                                        ),
                                    );
                                }
                    ...
                    let allow_soft = if !allow_hard && conf >= soft_thr {
                        ...
                        let recent: Vec<f32> = stable_vals
                            .iter()
                            .rev()
                    ...
                    let allow_major = if !allow_hard && !allow_soft {
                        ...
                        let mut counts: std::collections::HashMap<i32, usize> =
                            std::collections::HashMap::new();
                        for (v, t) in recent_ints_cand.iter().rev() {
                            if nowh.saturating_sub(*t) <= 1200 {
                                *counts.entry(*v).or_insert(0) += 1;
                            } else {
                                break;
                            }
                        ...
                        if let Some((k, c)) = best {
                            let need = if in_fast { 2 } else { 3 };
                            let prev_int = (*guard)
                                .and_then(|g| Some(g.bpm.round() as i32))
                                .unwrap_or(disp.round() as i32);
                    ...
                    if allow_hard || allow_soft || allow_major {
                        ...
                        if allow_soft {
                            if let Some(prev_int) = last_hard_int {
                                if prev_int == disp_int {
                                    show_state = last_hard_state.unwrap_or("tracking");
                                }
                            }
                            if in_fast {
                                if let Some(prev) = *guard {
                                    if prev.bpm.round() as i32 != disp_int {
                                        show_state = "uncertain";
                                    }
                                }
                            }
                        }
                        let payload = {
                            ...
                            if let Some((k, c)) = best {
                                let prev_int = (*guard)
                                    ...
                                    .unwrap_or(disp_int);
                                let need = if in_fast { 2 } else { 3 };
                                if (allow_soft || allow_hard || allow_major)
                                    && c >= need
                                    && k != prev_int
                                {
                                    emit_friendly(
                                        &app,
                                        format!("依据多数候选切换整数至 {} BPM", k),
                                        format!("Switched to majority integer {} BPM", k),
                                    );
                                    DisplayBpm {
                                        bpm: k as f32,
                                        confidence: conf,
                        ...
                        *guard = Some(payload);
                        let _ = app.emit_to("main", "bpm_update", payload);
                        if !allow_soft {
                            let changed = match last_hard_int {
                                Some(prev) => prev != disp_int,
                                None => true,
                            };
                            if changed {
                                emit_friendly(
                                    &app,
                                    format!("当前节拍：{} BPM", disp_int),
                                    format!("Current tempo: {} BPM", disp_int),
                                );
                            }
                            last_hard_int = Some(disp_int);
                            last_hard_state = Some(state);
                        }
                        let nowh = now_ms();
                        recent_ints.push_back((payload.bpm.round() as i32, nowh));
                        while let Some(&(_, t0)) = recent_ints.front() {
                            if nowh.saturating_sub(t0) > 1500 {
                                recent_ints.pop_front();
                            } else {
                                break;
                            }
                        }
            ...
            if tracking && conf >= 0.85 {
                if let Some(base) = anchor_bpm {
                        ...
                        if let Some(a) = anchor_bpm {
                            if EMIT_TEXT_LOGS {
                                let txt = if is_log_zh() {
                                    ...
                                    format!("[ANCHOR] anchor_bpm(update) -> {:.1}", a)
                                };
                                eprintln!("{}", txt);
                                let _ = app.emit_to(
                                    "main",
                            ...
                            emit_friendly(
                                &app,
                                format!("更新参考节拍：{:.1} BPM", a),
                                format!("Anchor updated: {:.1} BPM", a),
                            );
                        }
                    }
                } else {
                    if (60.0..=160.0).contains(&disp) {
                        ...
                        if EMIT_TEXT_LOGS {
                            let txt = if is_log_zh() {
                                format!("[锚点] 初始化 -> {:.1}", disp)
                            } else {
                                format!("[ANCHOR] anchor_bpm(init) -> {:.1}", disp)
                            };
                            eprintln!("{}", txt);
                            let _ = app.emit_to(
                                "main",
                        ...
                        emit_friendly(
                            &app,
                            format!("建立参考节拍：{:.1} BPM", disp),
                            format!("Anchor set: {:.1} BPM", disp),
                        );
                    }
                }
            }
            ...
            if conf >= 0.80 && EMIT_TEXT_LOGS {
                ...
                let txt = if is_log_zh() {
                    format!(
                        "[显示] 窗口={:.1}s 源={} bpm={:.1} 置信度={:.2} 状态={} 电平={:.2}",
            ...
            if EMIT_TEXT_LOGS {
                let txt = if is_log_zh() {
                    format!(
                        "[无结果] 本步无估计 电平={:.2} 追踪={} 连续空帧={}",
                        level, tracking, none_cnt
                    )
                } else {
                    format!(
                        "[NONE] win step no estimate, lvl={:.2} tracking={} none_cnt={}",
                        level, tracking, none_cnt
                    )
                };
                eprintln!("{}", txt);
            ...
            emit_friendly(
                &app,
                "暂未检测到清晰节拍，继续聆听…",
                "No clear beat yet. Listening…",
            );
            for _ in 0..hop_len {
                let _ = window.pop_front();
            }
            if none_cnt >= 6 {
                recent_none_flag = true;
            }
        }
    }
}
