...
无需虚拟声卡，默认读取电脑内部声音，也可切换为默认麦克风收音。
...
- 从电脑内部正在播放的声音中自动检测并显示 BPM，也可切换到麦克风收音。
...
- 看不到数值/总是 0？请确认当前收音源有声音、播放器音量或麦克风输入不为 0，或点击右上角刷新后再试。
- 为什么数字偶尔会小幅跳动？在切歌或过渡段属于正常现象，应用会很快稳定。
- 需要麦克风权限吗？默认电脑内部声音模式不需要；切到麦克风收音时会使用系统默认麦克风，可能需要 Windows 麦克风隐私权限。
